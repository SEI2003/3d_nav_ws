...
class ShutdownNode : public rclcpp::Node{
  public:
    ShutdownNode() : Node("shutdown_node"){
      publisher_ = this->create_publisher<std_msgs::msg::Bool>("/change_map",10);
    }

    void publishChangeMap(bool value){
      auto message = std_msgs::msg::Bool();
      message.data = value;
      publisher_->publish(message);
      RCLCPP_INFO(this->get_logger(),"Published /change_map: %s", value ? "ture":"false");
    }
...
class Nav2Client : public rclcpp::Node
{
  ...
  std::shared_ptr<ShutdownNode> shutdown_node_;
  explicit Nav2Client(size_t initial_count = 0): Node("nav2_send_goal"), count_(initial_count),renew_(0){
      this->declare_parameter<std::string>("map_file_path", "/default/path/map.yaml");
      this->get_parameter("map_file_path", file_path_);
      //アクション Client の作成
  ...
  void feedbackCallback(GoalHandleNavigateToPose::SharedPtr goal_handle, const std::shared_ptr<const NavigateToPose::Feedback> feedback){
    ...
    if (feedback->distance_remaining < 1.0 && renew_ == 1) {
      ...
      if (count_ < goal_points_.size()){
        ...
        shutdown_node_->publishChangeMap(true);

        rclcpp::shutdown();  // プログラムを終了
      }
    }
  }
...
int main(int argc, char **argv){
  ...
  auto shutdown_node = std::make_shared<ShutdownNode>();
  shutdown_node->publishChangeMap(true);
