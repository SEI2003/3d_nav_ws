...
class Nav2GoalClient(Node):
    def __init__(self):
        ...
        self.declare_parameter('map_file_path', '/home/user24/3d_navigation_ws/src/sirius_navigation/config/map.yaml')
        file_path = self.get_parameter('map_file_path').get_parameter_value().string_value
        self.get_logger().info(f'Map file path: {file_path}')
                # ファイルを開いて内容を読み取る
        try:
            with open(file_path, 'r', encoding='utf-8') as f:
                self.positions_list = yaml.safe_load(f)['points']  # pointsを直接取得
                self.get_logger().info(f"Loaded {len(self.positions_list)} points from {file_path}")
        except Exception as e:
            self.get_logger().error(f"Failed to load points from {file_path}: {str(e)}")
