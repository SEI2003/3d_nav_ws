# FlexBE States and Behaviors for kobuki
...
This raw repository has several folders and files with the generic name `kobuki`.
...
and change the relevant `kobuki` text to `my_new_project_name` as needed.
...
## Example States in `kobuki_flexbe_states`
...
## Example Behaviors in `kobuki_flexbe_behaviors`
...
[`copy_behavior`](kobuki_flexbe_behaviors/bin/copy_behavior) script to copy a saved behavior 
&mdash; both the Python implementation and manifest `.xml` file &mdash; 
to the project source folder for long term storage.
Use `ros2 run kobuki_flexbe_behavior copy_behavior` to see the usage guide. 
